...
export default async function ProfilePage({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  return <AccountShell title="Profile" copy="Keep your buyer profile details up to date.">{params.error ? <p className="mb-4 rounded-lg border border-rust/30 bg-rust/10 p-3 font-bold text-rust">{params.error}</p> : null}{params.saved ? <p className="mb-4 rounded-lg border border-success/30 bg-sage/10 p-3 font-bold text-success">Profile saved.</p> : null}<form action={saveProfileAction} className="max-w-2xl rounded-xl border border-line bg-white p-5"><div className="mb-5 grid h-20 w-20 place-items-center rounded-full bg-sand font-serif text-3xl">{(profile?.full_name || user.email || 'A').slice(0,1).toUpperCase()}</div><div className="grid gap-4"><Input name="full_name" label="Full name" defaultValue={profile?.full_name || ''}/><Input name="email" label="Email" defaultValue={profile?.email || user.email || ''} readOnly/><Input name="phone" label="Phone" defaultValue={profile?.phone || ''} required={false}/><button className="w-fit rounded-lg bg-rust px-5 py-3 font-black text-white">Save changes</button></div></form></AccountShell>;
}
