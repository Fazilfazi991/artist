...
function ShopPromos({ categories }: { categories: Awaited<ReturnType<typeof getFeaturedCategories>> }) {
  ...
  return <section className="mb-8 grid gap-4 lg:grid-cols-[1.4fr_.9fr]">
    <Link href="/shop?sort=newest" className="group relative min-h-[260px] overflow-hidden rounded-2xl border border-line bg-ink p-7 text-white shadow-soft">
      ...
      <div className="relative z-10 max-w-lg">
        ...
        <h2 className="mt-5 font-serif text-5xl leading-tight">Fresh handmade finds for modern homes.</h2>
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
      <Link href="/shop?type=bespoke" className="rounded-2xl border border-line bg-rust p-6 text-white shadow-soft transition hover:-translate-y-0.5"><BadgeCheck size={24} /><h3 className="mt-4 font-serif text-3xl">Custom orders</h3><p className="mt-2 text-sm leading-6 text-white/80">Request personalized pieces and approve a quote before work begins.</p></Link>
      <Link href="/shop?sort=featured" className="rounded-2xl border border-line bg-surface p-6 shadow-soft transition hover:-translate-y-0.5"><PackageCheck size={24} className="text-rust" /><h3 className="mt-4 font-serif text-3xl">Ready to gift</h3><p className="mt-2 text-sm leading-6 text-muted">Find popular products with tracked delivery and protected online payment.</p></Link>
    </div>
    {featured.length ? <div className="lg:col-span-2 rounded-2xl border border-line bg-white p-4 shadow-soft">
      <div className="flex flex-wrap items-center gap-2">
        <span className="mr-2 text-sm font-black uppercase tracking-[.12em] text-muted">Featured categories</span>
        {featured.map((category) => <Link key={category.slug} href={`/shop?category=${category.slug}`} className="rounded-full border border-line bg-paper px-4 py-2 text-sm font-black transition hover:border-rust hover:text-rust">{category.name}</Link>)}
      </div>
    </div> : null}
  </section>;
}
