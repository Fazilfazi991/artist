    ...
    <section className="border-b border-line bg-surface-low">
      <div className="heritage-container py-16">
        ...
        <div className="mt-4 grid gap-6 lg:grid-cols-[1fr_360px] lg:items-end">
          <div>
            <h1 className="max-w-3xl font-serif text-5xl font-semibold leading-tight text-ink sm:text-7xl">Discover independent makers and their stories.</h1>
...
export function StorefrontCardView({ storefront, compact = false }: { storefront: StorefrontCard; compact?: boolean }) {
  return <article className="overflow-hidden rounded-xl border border-line bg-surface shadow-soft transition hover:-translate-y-0.5 hover:shadow-lift">
    <div className="relative aspect-[16/10] bg-surface-mid">
      ...
      <div className="absolute bottom-4 left-4 flex items-end gap-3">
        ...
        <span className="pb-1 text-white"><strong className="block font-serif text-2xl font-semibold leading-tight">{storefront.storeName}</strong><span className="text-xs font-bold text-white/85">{storefront.city}, {storefront.state}</span></span>
      </div>
    </div>
    <div className="p-5">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 text-sm font-bold text-muted">
        <span className="flex items-center gap-2 rounded bg-paper px-3 py-2"><Boxes size={16} className="text-rust" />{storefront.productCount} products</span>
        <span className="flex items-center gap-2 rounded bg-paper px-3 py-2"><Layers3 size={16} className="text-rust" />{storefront.collectionCount} collections</span>
      </div>
      <div className="mt-5 flex flex-col gap-2 sm:flex-row">
        <Link href={`/artisan/${storefront.storeSlug}`} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded bg-rust px-4 text-sm font-extrabold text-white">Visit Storefront <ArrowRight size={15} /></Link>
        {storefront.customOrdersEnabled ? <Link href={`/artisan/${storefront.storeSlug}/custom-order`} className="inline-flex min-h-11 flex-1 items-center justify-center rounded border border-line bg-paper px-4 text-sm font-extrabold text-ink">Request Custom Order</Link> : null}
      </div>
    </div>
  </article>;
}
